                ...
                <div class="border border-gray-200 rounded-lg p-5 sm:p-7">
                    ...
                    <div class="bg-gray-50 border border-gray-100 rounded-lg px-4 py-3 mb-6 flex gap-3 items-start">
                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="#111"
                            stroke-width="1.8" width="16" height="16" class="flex-shrink-0 mt-0.5">
                            <path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" />
                            <circle cx="12" cy="10" r="3" />
                        </svg>
                        <div>
                            <p class="text-xs font-bold text-gray-900">{{ $address->name }}</p>
                            <p class="text-xs text-gray-500 mt-0.5">{{ $address->street }}, {{ $address->region }} {{
                                $address->postal }}</p>
                            <p class="text-xs text-gray-400 mt-0.5">Kurir: <span class="font-semibold text-gray-700">{{
                                    strtoupper($shipping) }}</span> · Ongkir: <span
                                    class="font-semibold text-gray-700">Rp {{ number_format($shippingCost, 0, ',', '.')
                                    }}</span></p>
                        </div>
                    </div>

                    <form action="{{ route('pelanggan.checkout.payment.simpan') }}" method="POST" id="paymentForm"
                        ...
                        <div id="form-bank_transfer" class="payment-block">
                            <div class="mb-4">
                                ...
                                <select name="bank_provider" id="bankProvider" class="form-select">
                                    ...
                                    <option value="{{ is_array($bank) ? $bank['value'] : $bank->value }}">
                                        {{ is_array($bank) ? $bank['label'] : $bank->label }} — {{ is_array($bank) ?
                                        $bank['number'] : $bank->number }} a.n {{ is_array($bank) ? $bank['name'] :
                                        $bank->name }}
                                    </option>
                                    @endforeach
                                </select>
                            ...
                            <div id="bankDetail" class="hidden bg-gray-50 border border-gray-100 rounded-lg p-4 mb-4">
                                <p class="text-xs text-gray-400 uppercase font-bold tracking-widest mb-2">Detail
                                    Rekening</p>
                                <p class="text-sm font-bold text-gray-900" id="bankDetailName">—</p>
                                <p class="text-lg font-extrabold text-gray-900 tracking-widest mt-1"
                                    id="bankDetailNumber">—</p>
                                <p class="text-xs text-gray-500 mt-0.5">a.n. <span id="bankDetailHolder">TANKEN
                                        ID</span></p>
                            </div>
                            ...
                            <div class="flex flex-col items-center py-4">
                                ...
                                <div class="qris-placeholder mb-3">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke="#9ca3af" stroke-width="1.2" width="48" height="48">
                                ...
                                <p class="text-[11px] text-gray-400 mt-1">Scan QR code ini dengan aplikasi dompet
                                    digital apapun</p>
                            </div>
                        ...
                        <div class="mt-7 pt-6 border-t border-gray-100">
                            <label class="form-label mb-3">Upload Bukti Pembayaran <span
                                    class="text-red-500">*</span></label>

                            <div class="upload-zone" id="uploadZone">
                                <input type="file" name="payment_proof" id="paymentProof" class="upload-input"
                                    accept=".jpg,.jpeg,.png">

                                <div id="uploadUiDefault" class="flex flex-col items-center">
                                    <div
                                        class="w-11 h-11 rounded-full bg-gray-100 flex items-center justify-center mb-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke="currentColor" stroke-width="1.8" class="w-5 h-5 text-gray-500">
                                ...
                                <div id="uploadUiSuccess" class="hidden flex flex-col items-center">
                                    <div
                                        class="w-11 h-11 rounded-full bg-green-100 flex items-center justify-center mb-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke="currentColor" stroke-width="2" class="w-5 h-5 text-green-600">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7" />
                                        </svg>
                                    </div>
                                    <p class="text-sm font-semibold text-green-700 truncate max-w-[200px]"
                                        id="fileNameDisplay">—</p>
                            ...
                            <p id="uploadSizeMsg" class="hidden text-xs font-medium text-red-500 mt-2">
                                <i class="fa-solid fa-circle-exclamation mr-1"></i> Ukuran file terlalu besar. Maksimal
                                3 MB.
                            </p>
                        </div>
                ...
                <div class="summary-card">
                    ...
                    <div class="flex flex-col gap-2.5 text-sm">
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-semibold text-gray-800">Rp {{ number_format($shippingCost, 0, ',', '.')
                                }}</span>
                        </div>
                    ...
                    <div class="flex justify-between items-center">
                        <span class="font-extrabold text-gray-900">Total Bayar</span>
                        <span class="font-extrabold text-gray-900 text-lg">
                            Rp {{ number_format($totalPayment, 0, ',', '.') }}
                        </span>
                    </div>
                    <div class="bg-amber-50 border border-amber-100 rounded-lg p-3 mt-3">
                    <p class="text-[11px] text-amber-700 leading-relaxed">
                        <i class="fa-solid fa-triangle-exclamation mr-1"></i>
                        Transfer <span class="font-extrabold">tepat sesuai nominal</span> di atas ya — angka unik di belakang membantu kami verifikasi pembayaranmu lebih cepat.
                    </p>
                </div>
            </div>
