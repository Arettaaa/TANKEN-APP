    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .filter-section {
            ...
            border-radius: 8px; display: flex; justify-content: space-between; align-items: center; margin-bottom: 24px;
        }
        ...
        /* =========================================
           TABLE STOK (RAPID & PRESISI)
           ========================================= */
        .table-container { 
            background-color: var(--card-bg); 
            border: 1px solid var(--border-color); 
            border-radius: 8px; 
            overflow-x: auto; /* Memastikan tabel tidak rusak di layar kecil */
        }
        
        table { 
            width: 100%; 
            border-collapse: collapse; 
            min-width: 1000px; /* Lebar minimum agar tabel tidak saling menjepit */
        }
        ...
        th { 
            ...
            white-space: nowrap; /* Mencegah judul kolom turun ke bawah */
        }
        ...
        tr:hover td { background-color: #FAFAFA; } /* Efek hover halus pada baris */

        /* PENGUNCIAN LEBAR KOLOM (KUNCI KERAPIAN) */
        th:nth-child(1) { width: 35%; } /* Kolom Produk (Paling Lebar) */
        th:nth-child(2) { width: 15%; } /* Kolom SKU */
        th:nth-child(3) { width: 10%; } /* Kolom Kategori */
        th:nth-child(4) { width: 12%; } /* Kolom Stok */
        th:nth-child(5) { width: 10%; } /* Kolom Min Stok */
        th:nth-child(6) { width: 10%; } /* Kolom Status */
        th:nth-child(7) { width: 8%; }  /* Kolom Aksi */

        /* Mencegah teks terlipat berantakan */
        .td-sku, .td-stock, .td-min-stock, .td-status, .td-action {
            white-space: nowrap; 
        }

        .td-product { display: flex; align-items: center; gap: 16px; }
        .product-img { width: 44px; height: 44px; background-color: #eee; border-radius: 6px; object-fit: cover; flex-shrink: 0; }
        .product-name { font-weight: 600; color: var(--text-main); line-height: 1.4; }

        .td-sku { color: var(--text-muted); font-family: monospace; font-size: 13px; }
        .td-stock { font-size: 15px; font-weight: 800; }
        .td-min-stock { color: var(--text-muted); font-weight: 600; }
        
        .badge-cat { padding: 4px 10px; border-radius: 20px; font-size: 12px; font-weight: 600; background-color: #e0f2fe; color: #0284c7; display: inline-block; }
        ...
        .status-aman { background-color: var(--icon-green-bg); color: var(--icon-green-text); border: 1px solid #bbf7d0; }
        .status-menipis { background-color: var(--icon-orange-bg); color: var(--icon-orange-text); border: 1px solid #fed7aa; }
        ...
        .btn-action-small {
            ...
            display: flex; align-items: center; justify-content: center; gap: 4px; transition: 0.2s; color: var(--text-main);
        }
        .btn-action-small:hover { background: var(--main-bg); border-color: var(--text-light); }
        .btn-action-small.in { color: #16a34a; } /* Hijau untuk masuk */
        .btn-action-small.out { color: #dc2626; } /* Merah untuk keluar */
    </style>
</head>
<body>
                ...
                <table id="stokTable">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="td-kategori"><span class="badge-cat wanita">Wanita</span></td>
                            <td class="td-stock">44</td>
                            <td class="td-min-stock">10</td>
                            <td class="td-status"><span class="status-badge status-aman">Aman</span></td>
                            <td class="td-action">
                                <div class="action-btns">
                                    <button class="btn-action-small in"><i class="ph ph-plus"></i> In</button>
                                    <button class="btn-action-small out"><i class="ph ph-minus"></i> Out</button>
                                </div>
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td class="td-kategori"><span class="badge-cat">Pria</span></td>
                        ...
                        <tr>
                            <td>
                                <div class="td-product">
                                    <img src="https://images.unsplash.com/photo-1506629082955-511b1aa562c8?q=80&w=200" alt="Produk" class="product-img">
                                    <span class="product-name">Cargo Shortpants Wanita Nylon</span>
                                </div>
                            </td>
                            ...
                            <td class="td-min-stock">10</td>
                            <td class="td-status"><span class="status-badge status-menipis">Menipis</span></td>
                            <td class="td-action">
                                <div class="action-btns">
                                    <button class="btn-action-small in"><i class="ph ph-plus"></i> In</button>
                                    <button class="btn-action-small out"><i class="ph ph-minus"></i> Out</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // 1. FUNGSI UPDATE JUMLAH ITEM TERLIHAT
            ...
            // 2. FUNGSI UTAMA UNTUK MENJALANKAN FILTER & SEARCH
            function jalankanFilter() {
                const keyword = searchInput.value.toLowerCase();
                const kategori = filterKategori.value.toLowerCase();
                const urutan = filterUrutkan.value;

                // Tahap 1: Saring berdasarkan Search dan Kategori
                rows.forEach(row => {
                ...
                // Tahap 2: Lakukan Pengurutan (Sorting)
                if (urutan !== "") {
                    // Hanya ambil baris yang tidak disembunyikan
            ...
            // Pasang Event Listener agar filter berjalan otomatis saat ada perubahan
